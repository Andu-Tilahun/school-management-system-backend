package com.schoolmanagment.coreservice.classroom.specification;
...
public class ClassRoomSpecification implements Specification<ClassRoom> {
    ...
    private final ClassRoomFilterRequest filterRequest;
    ...
    public Predicate toPredicate(Root<ClassRoom> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (filterRequest.getSearchText() != null && !filterRequest.getSearchText().isBlank()) {
            ...
            predicates.add(cb.like(cb.lower(root.get("roomNumber")), likeValue));
        }
    ...
    private void applySorting(Root<ClassRoom> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
